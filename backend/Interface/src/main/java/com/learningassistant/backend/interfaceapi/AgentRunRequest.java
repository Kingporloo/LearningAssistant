...
public record AgentRunRequest(
        ...
        @JsonProperty("runtime_config") RuntimeConfig runtimeConfig) {
    ...
    public record RuntimeConfig(
            String model,
            String persona,
            @JsonProperty("enabled_tools") List<String> enabledTools) {

        public RuntimeConfig {
            model = required(model, "model");
            persona = persona == null ? "" : persona.strip();
            if (persona.length() > 2_000) {
                throw new IllegalArgumentException("persona 不能超过 2000 个字符");
            }
            if (enabledTools == null) {
                throw new IllegalArgumentException("enabledTools 不能为空");
            }
