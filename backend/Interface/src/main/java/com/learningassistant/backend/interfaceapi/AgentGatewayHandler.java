...
import com.learningassistant.backend.dataport.DataPortException;
import com.learningassistant.backend.dataport.DocumentDataPort;
import com.learningassistant.backend.dataport.RagDocumentStore;
import com.learningassistant.backend.dataport.SessionLedgerDataPort;
import com.learningassistant.backend.user.UserRequestHandler;
import com.learningassistant.backend.user.UserService;
import com.learningassistant.backend.user.UserServiceException;
...
final class AgentGatewayHandler {
    ...
    private final AgentSettingsService settings;
    ...
    private void route(HttpExchange exchange, String path, String userId) {
        ...
        if ("/agent/settings".equals(path)) {
            handleAgentSettings(exchange, method, userId);
            return;
        }
    ...
    private void handleAgentSettings(HttpExchange exchange, String method, String userId) {
        if ("GET".equals(method)) {
            http.send(exchange, 200, mapper.valueToTree(settings.get(userId)));
            return;
        }
        if (!"PUT".equals(method)) {
            throw new AgentGatewayException(405, "请求方法不受支持");
        }
        var body = http.readObject(exchange, Set.of("model", "persona", "enabled_tools"));
        var toolsNode = body.get("enabled_tools");
        if (toolsNode == null || !toolsNode.isArray()) {
            throw new AgentGatewayException(400, "enabled_tools 必须是字符串数组");
        }
        var tools = new ArrayList<String>();
        for (var item : toolsNode) {
            if (!item.isTextual()) {
                throw new AgentGatewayException(400, "enabled_tools 必须是字符串数组");
            }
            tools.add(item.textValue());
        }
        JsonNode personaNode = body.get("persona");
        if (personaNode == null || !personaNode.isTextual()) {
            throw new AgentGatewayException(400, "persona 必须是字符串");
        }
        http.send(exchange, 200, mapper.valueToTree(settings.update(
                userId,
                requiredText(body, "model", 128),
                personaNode.textValue(),
                tools)));
    }
